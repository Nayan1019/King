    ...
    if (type === 'message' && senderID === ADMIN_UID) {
      ...
      if (lower.startsWith(`${PREFIX}groupnamelock on`)) {
        ...
        try {
          ...
          api.sendMessage(` ✅GC name locked to "${name}" successfully.🔒`, threadID);
        } catch (e) {
          api.sendMessage(`❌ Failed to lock name: ${e.message}`, threadID);
        }
      }
      ...
      if (lower.startsWith(`${PREFIX}nicknamelock on`)) {
        ...
        if (!nickname) return api.sendMessage(`❌ Usage:\n${PREFIX}nicknamelock on <nickname>`, threadID);

        api.getThreadInfo(threadID, async (err, info) => {
          if (err) return api.sendMessage("❌ Failed to get members.", threadID);
          ...
          api.sendMessage(`🔒 All nicknames locked to "${nickname}" successfully.`, threadID);
        });
      }
      ...
      if (lower === `${PREFIX}nicknamelock off`) {
        if (lockedNicknames[threadID]) {
          ...
          api.sendMessage("⚠️ No nickname lock is active.", threadID);
        }
      }
    }
